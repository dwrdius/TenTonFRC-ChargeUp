...
void limitSwerveSpeeds (double speed)
{
    ...
    mathConst::pseudoSpeedLimit = speed;
}
    ...
    if (m_chooser.GetSelected() == kCubeConeAlignLeft || m_chooser.GetSelected() == kCubeConeAlignRight) {
        if(coneCollect){
            ...
            else if (autoIndex == 2)
            {
                ...
                if (deadband(ty, 5) < 0)
                {
                    LLAlignTranslation = -deadband(ty-0.8, 5)/30;
                }
            ...
            else if (autoIndex == 5 || autoIndex == 6)
            {
                IntakeUpDown.Set(TalonFXControlMode::PercentOutput, 0);
        ...
        if(balance){
            ...
            if (abs(roll)>6 && !onChargingStation)
            {
                onChargingStation = true;
                heresyTimer = 0;
            }
            else if (onChargingStation)
            {
                roll = deadband(navX.GetRoll()/4, 3)*4;
                roll = -roll / 5;
            ...
            frc::SmartDashboard::PutNumber("Intake Error", intakePos-IntakeUpDown.GetSelectedSensorPosition());
            if (intakePos - IntakeUpDown.GetSelectedSensorPosition() < 500 && !deadband(ty, 3))
            {
                intakePercentage = 0;
            }
            intakeRampSpeed = slew(intakeRampSpeed, intakePercentage, 1);
        ...
        if(balance){      
            ...
            else if (autoIndex == 2)
            {
                ...
                autoDesiredStates = swerveKinematics(0, 0, diffAngle, 0, false);
                heresyTimer++;
                if (!deadband(diffAngle, 3) || heresyTimer > 100)
                {
                    autoIndex = 3;
                }
                frc::SmartDashboard::PutNumber("diffangle", diffAngle);
            }
            // else if (autoIndex == 3)
            // {
            //     heresyTimer++;
...
void Robot::TeleopInit() 
{
    ...
    limitSwerveSpeeds(mathConst::speedLimit);
    navX.ZeroYaw();
...
void Robot::TeleopPeriodic()
{
    ...
    //     { 
    //         IntakeUpDown.Set(ControlMode::PercentOutput, -0.5);
    //     }
    //     else
    //     {
    //         IntakeUpDown.Set(ControlMode::PercentOutput, 0);
    ...
    else
    {
        ...
        else if (controllerAux.GetPOV()==180)
        {
            armState = 6;
        }
...
// Debug Math Outputs
    ...
    if(controller.GetLeftTriggerAxis())
    {
        if(throttle==false)
        {
            limitSwerveSpeeds(mathConst::speedLimit/2);
            throttle = true;
        }
    }
    else
